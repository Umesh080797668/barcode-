...
export default function App() {
  ...
  const handleScannedBarcode = useCallback(async (barcode, source = 'scan') => {
    ...
    if (activeTab === 'billing' || (activeTab === 'data' && ['normal', 'return', 'used_purchase'].includes(inventoryAddMode))) {
      let tMode;
      if (activeTab === 'data') {
        if (inventoryAddMode === 'return') tMode = 'customer_return';
        else if (inventoryAddMode === 'used_purchase') tMode = 'used_purchase';
      }
      if (activeTab !== 'billing') setActiveTab('billing');
      window.postMessage({ action: 'billing:scan', barcode, transactionMode: tMode }, '*');
      return;
    }
    await processBarcode(barcode, source);
                  ...
                  <div className="inventory-mode-card">
                    <div className="mode-toggle-row">
                      ...
                      <button className={`mode-chip ${inventoryAddMode === 'normal' ? 'active' : ''}`} onClick={() => setInventoryAddMode('normal')} type="button">
                        <span className="mode-chip-title">Normal Sale</span>
                        <span className="mode-chip-sub">Scan goes to billing</span>
                      </button>
                      <button className={`mode-chip ${inventoryAddMode === 'return' ? 'active' : ''}`} onClick={() => setInventoryAddMode('return')} type="button">
                        <span className="mode-chip-title">Customer Return</span>
                        <span className="mode-chip-sub">Scan takes item back</span>
                      </button>
                      <button className={`mode-chip ${inventoryAddMode === 'used_purchase' ? 'active' : ''}`} onClick={() => setInventoryAddMode('used_purchase')} type="button">
                        <span className="mode-chip-title">Used Purchase</span>
                        <span className="mode-chip-sub">Scan buys used item</span>
                      </button>
                    </div>
                    <div className="field-hint" style={{ marginTop: 8 }}>Selected mode controls how the next scan behaves.</div>
                  </div>
                </section>
              )}
